...
def simulate(l, b, beamwidth=0.6, v_min=-400, v_max=400, plot_file=''): 
	...
	try:
		...
		raise requests.exceptions.ConnectionError('Failed to reach astro.uni-bonn.de.')

	data = response.content.splitlines()[4:]
	frequency = [float(line.split()[2]) for line in data if len(line.split()) > 2]
	spectrum = [float(line.split()[1]) for line in data if len(line.split()) > 2]

	v_min, v_max = v_min*1000, v_max*1000
	...
	l, b = float('%.2f' % l), float('%.2f' % b)
	fig, ax = plt.subplots()

	plt.title(r'Simulated HI Profile $(l=' + str(l) + r'^\circ, b=' + str(b) + r'^\circ)$ | Beamwidth: $' + str(beamwidth) + r'^\circ$', pad=40)
	...
	if plot_file != '': plt.savefig(plot_file, bbox_inches='tight', pad_inches=0.1)
	else: plt.show()
	plt.close()
...
def predict(lat, lon, height=0, source='', date='', plot_sun=True, plot_file=''):
	...
	obj = SkyCoord.from_name(source) if source != '' else None
	loc = EarthLocation(lat=lat*u.deg, lon=lon*u.deg, height=height*u.m)

	offset = time.timezone if (time.localtime().tm_isdst == 0) else time.altzone
	utcoffset = (offset / 3600 * -1) * u.hour
	...
	if obj:
		...
		# CORRECTION ICI : Conversion explicite en valeur scalaire sans unité
		seconds = (delta_midnight[np.argmax(obj_altaz.alt)].to(u.hour).value)
		plt.axvline(x=seconds, color='brown', linestyle='--', linewidth=2, zorder=2)
		plt.scatter(delta_midnight.value, obj_altaz.alt.degree, c=obj_altaz.az.degree, label=source, s=8, cmap='viridis')

	if plot_sun:
		sun_altaz = get_sun(full_day).transform_to(frame)
		plt.scatter(delta_midnight.value, sun_altaz.alt.degree, c=sun_altaz.az.degree, label='Sun', s=8, cmap='viridis')

	plt.colorbar(label='Azimuth (deg)', aspect=40)
	plt.legend(loc='best')
	plt.xlim(0, 24)
	plt.ylim(0, 90)
	plt.xlabel(f'Time (UTC{int(utcoffset.value):+d})')
	plt.ylabel('Altitude (deg)')
	plt.title(f'{source} | {date}')
...
def plot(obs_parameters='', n=0, m=0, f_rest=0, slope_correction=False, dB=False, vlsr=False, meta=False, 
         avg_ylim=[0,0], cal_ylim=[0,0], rfi=[], xlim=[0,0], ylim=[0,0], dm=0,
